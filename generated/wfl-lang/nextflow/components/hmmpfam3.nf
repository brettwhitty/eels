...
process hmmpfam3 {
    script:
    """
    ${params.hmmscan_exec} ${params.other_opts} ${params.cutoffs} ${params.database_path} ${params.i_file_path} \
        1>${params.output_directory}/${params.iterator_name}/g${params.group_number}/${params.i_file_base}.raw
    """
}

process hmmpfam32htab {
    script:
    """
    ${params.bin_dir}/hmmpfam32htab --donotrun ${params.skip_htab} --input_file ${params.output_directory}/${params.iterator_name}/g${params.group_number}/${params.i_file_base}.raw --output_htab ${params.output_directory}/${params.iterator_name}/g${params.group_number}/${params.i_file_base}.htab --mldbm_file '${params.hmm_info_file}' \
        2>{{OUTPUT_DIRECTORY}}/{{ITERATOR_NAME}}/g{{GROUP_NUMBER}}/{{I_FILE_BASE}}.stderr
    """
}

process hmmpfam32bsml {
    script:
    """
    ${params.bin_dir}/hmmpfam32bsml --input ${params.output_directory}/${params.iterator_name}/g${params.group_number}/${params.i_file_base}.raw --output ${params.output_directory}/${params.iterator_name}/g${params.group_number}/${params.i_file_base}.bsml --fasta_input ${params.i_file_path} --gzip_output ${params.compress_bsml_output} --model_mol_type ${params.model_mol_type} --model_mol_class ${params.model_mol_class} --query_mol_type ${params.query_mol_type} --query_mol_class ${params.query_mol_class}
    """
}
