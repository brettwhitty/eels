...
process wu_blastp {
    script:
    """
    ${params.blastp_exec} ${params.database_path} ${params.i_file_path} -E ${params.expect} -matrix ${params.matrix} -wordmask ${params.filter} -B ${params.database_matches} -V ${params.descriptions} -gspmax ${params.gspmax} -shortqueryok -novalidctxok ${params.other_opts} \
