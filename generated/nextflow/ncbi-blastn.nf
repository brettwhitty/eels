...
process ncbi_blastn {
    script:
    """
    ${params.blastall_exec} -p blastn -i ${params.i_file_path} -d ${params.database_path} -e ${params.expect} -F '${params.filter}' -b ${params.database_matches} -v ${params.descriptions} ${params.other_opts} \
